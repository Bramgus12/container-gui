...
final class CLIDecodingTests: XCTestCase {
    ...
    func testDecodesCurrentContainerFixtureAndIgnoresUnknownFields() throws {
        ...
        XCTAssertEqual(details.summary.publishedPorts.first?.hostPort, 8_080)
        XCTAssertEqual(details.summary.mounts.first?.destination, "/data")
        XCTAssertEqual(details.summary.networkNames, ["default"])
        XCTAssertNil(details.summary.startedAt)
