...
private func makeRelease(
    tag: String,
    name: String = "CargoDeck",
    notes: String = "",
    page: String = "https://github.com/Bramgus12/CargoDeck/releases/tag/v9.9.9"
) throws -> AppRelease {
...
final class AppReleaseDecodingTests: XCTestCase {
    ...
    /// `/repos/Bramgus12/CargoDeck/releases/latest` endpoint, so a change in
    ...
    func testDecodesCapturedGitHubReleasePayload() throws {
        ...
        XCTAssertEqual(release.name, "CargoDeck 1.1.0")
        XCTAssertEqual(
            release.pageURL.absoluteString,
            "https://github.com/Bramgus12/CargoDeck/releases/tag/v1.1.0"
        )
...
final class GitHubUpdateServiceTests: XCTestCase {
    ...
    func testDecodesASuccessfulResponse() async throws {
        ...
        {"tag_name":"v2.0.0","name":"CargoDeck 2.0.0","body":"Notes","html_url":"https://example.com/r"}
        """)
        let release = try await service.latestRelease()
        XCTAssertEqual(release.version, SemanticVersion(major: 2, minor: 0, patch: 0))
        XCTAssertEqual(release.name, "CargoDeck 2.0.0")
    }
